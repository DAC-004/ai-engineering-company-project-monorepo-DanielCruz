...
def main() -> int:
    ...
    approved_total_cost_frontend_changes = {
        "uis/talent-pipeline-tracker/components/inventory/SupplyDeliveryForm.tsx",
        "uis/talent-pipeline-tracker/lib/telemetry/inventoryEvents.ts",
        "uis/talent-pipeline-tracker/lib/telemetry/schema.ts",
    }
    unexpected_frontend_changes = sorted(
        set(frontend_changed) - approved_total_cost_frontend_changes
    )
    specs_changed = [path for path in all_changed if path.startswith(".project_specs/")]
    check(
        "no unexpected frontend changes beyond approved total_cost capture",
        unexpected_frontend_changes == [],
        str(unexpected_frontend_changes),
    )
    check(
        ".project_specs is not in the implementation diff",
        specs_changed == [],
        str(specs_changed),
    )
