...
def bulk_insert_telemetry_events(session: Session, rows: list[dict[str, Any]]) -> int:
    """Insert accepted rows in one statement. Return the number actually stored.

    Conflict-skipped event_id values are not stored and must be counted as
    rejected by the caller (received - stored). Unexpected database errors
    propagate so the handler does not return HTTP 200 with false stored counts.
    """
...
def persist_telemetry_batch(
    ...
    """Validate each raw item, collect accepted rows, and bulk-insert once.

    ``received`` is the raw list length. ``stored`` is the number of rows
    actually inserted. ``rejected`` is received minus stored, covering
    validation failures, timestamp conversion failures, same-batch duplicate
    IDs, and already-stored IDs.
    """
