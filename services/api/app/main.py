...
# manager_constants is imported by name from packages/shared. Inserting that
# directory does not replace the repo-root `shared` package or types/index.ts.
...
# Analyzer static paths (/analyze, /results) must be registered before /{incident_id}.
...
def _is_manager_write_request(request: Request) -> bool:
    """True for manager create/status bodies that must return HTTP 400."""
